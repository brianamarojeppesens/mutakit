...
async function buildAll() {
  ...
  if (!CHECK) {
    ...
    await syncSizeAccounting(outputs);
  }
  return report;
}

/**
 * Rewrite the preset table in `docs/size-accounting.md` from this build.
 *
 * The document's whole authority is that its numbers are measured, and a
 * hand-copied measurement stops being one the first time anything ships: the
 * table claimed 67.47 KB for the full preset for as long as it took four
 * subsystems to land. So the build owns the region between the markers and
 * nothing else in the file is touched — the argument stays prose, written by
 * hand; only the figures it rests on are generated.
 */
async function syncSizeAccounting(outputs) {
  const file = path.join(ROOT, "docs/size-accounting.md");
  let doc;
  try {
    doc = await readFile(file, "utf8");
  } catch {
    return; // The document is optional; the build is not.
  }
  ...
  const rows = outputs
    .filter((o) => o.format === "iife" && o.minified)
    .map((o) => `| \`${o.preset}\` | ${kb(o.bytes)} | ${kb(o.gzip)} |`);
  const table = [
    "",
  ...
  if (next !== doc) await writeFile(file, next);
}
