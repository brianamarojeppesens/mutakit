...
2. Core (kernel + geometry + engine + the `anchor` and `stack` algorithms) is ≤ 8.5 KB
   gzipped — see §20.5 for the per-module accounting behind that number.
...
types — `pane`, `surface`, `stack`, `group`, `spacer` (Tier A in §11). Budget: **≤ 8.5 KB
gzipped**, with the per-module accounting in §20.5.
...
| Metric | Target |
|---|---|
| Core bundle | **≤ 8.5 KB gzipped** (~26 KB minified) |
| Full bundle | **≤ 32 KB gzipped** (~100 KB minified) |
...
1. **The ≤ 20 KB minified budget was not achievable** and is revised to ~26 KB / 8.5 KB
   gzipped. The gzip figure was always the meaningful one and is comfortably met, so this
   is a correction to the *metric*, not a real regression in ambition.
